...
public class BasicCalculator2 {
    ...
    public static void main(String[] args) {
        ...
        String s1 = " 3/2 "; // 1
        String s3 = " 3+5 / 2"; //5
        //System.out.println(calculateUsingStack(s));
    ...
    static int calculateWithoutStack(String s) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!Character.isDigit(currentChar) && !Character.isWhitespace(currentChar)
                    || i == length - 1) { //This condition is always executed at the end of the string
    ...
    Space: O(n)
     */
    static int calculateUsingStack(String s) {
        if (s == null || s.isEmpty()) return 0;
        int len = s.length();
        ...
        for (int i = 0; i < len; i++) {
            char currentChar = s.charAt(i);
            if (Character.isDigit(currentChar)) {
                                // multiply by 10 for this case: 35+2. We want to add 30 + 2 for the index-0 -> 1
                currentNumber = (currentNumber * 10) + (currentChar - '0');
            }
